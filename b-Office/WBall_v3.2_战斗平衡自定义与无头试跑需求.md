...
> 状态：**已交付并通过自动验收（2026-07-27）**。实现与标定记录见 §11。
...
- M1~M7 已落地：独立平衡配置、全部 `balance.*` 命令、剧本 balance 段、隔离式无头试跑、预设档、「战斗平衡」工具窗与对战台入口。
- 三项默认玩法变更已启用：40k 起等比升格（2/4/8…封顶 64）、取消 512 队列上限并保留 100 万防 OOM 硬顶、护盾自然再生默认 0。
- 无头试跑支持多种子、table/CSV、整批墙钟超时、取消后返回已完成部分；试跑使用独立世界与配置副本，不污染现场。
- `rush` 经首轮数据发现慢于 `standard` 后，加入 120 秒硬时限并重新标定；内置预设使用 `.seeded-v3.2-calibrated` 一次性迁移标记。

### 11.2 自动验收

构建结果：`0` 警告、`0` 错误；`WBallVerify` 全部通过。覆盖升格档形、超 512 队列、队列积分增量值、护盾禁再生/显式开启、满盾巨球触杀、硬时限、预设/剧本往返、试跑确定性与现场隔离、取消部分结果，以及命令层烟测。

```text
v3.1 rollback seed=42 @60s
6381A3898C0FAD65B57D43C140917A010713AA3015F601BACE14C7E5B88333F3

v3.2 default seed=42 @60s
E24FD280C34B54F79DAFCAE466DE299B4B76F56B69D83EF63757B96F81BF9184

v3.2 default seed=43 @60s
436DAEA13BE0430B1C2513DC0D22DB04047352E14711673BC26451490A071554
```

### 11.3 三档 8 种子标定

统一条件：`demo4` 四炮台、种子 `42..49`、每局上限 180 逻辑秒。首轮 24 局并行墙钟约 378 秒；这是约 11.4 倍于实时总量的无头吞吐。

| 预设 | 平均(s) | 中位(s) | 最小/最大(s) | 胜者分布 | 平局率 | 超时率 |
|---|---:|---:|---:|---|---:|---:|
| `standard` | 125.890 | 120.667 | 75.550 / 180.000 | cyan 2、green 1、magenta 3、timeout 2 | 0% | 25% |
| `rush`（校准后） | 108.894 | 120.000 | 67.733 / 120.000 | cyan 3、magenta 1、orange 4 | 0% | 0% |
| `marathon` | 142.971 | 180.000 | 61.033 / 180.000 | cyan 3、timeout 5 | 0% | 62.5% |

硬时限判定使用浮点 epsilon，`rush` 在 120.000 秒严格收敛、不多跑一帧。三档呈现明确层级：rush 更快且必收敛，standard 居中，marathon 更容易形成长局。

### 11.4 实机状态

主程序已成功启动并显示 `WBall v3.2.0`；窗口自动检查因检测到用户正在操作而主动停止，未抢占焦点。工具窗注册与命令入口由构建及命令烟测覆盖，最终视觉布局保留人工验收。
